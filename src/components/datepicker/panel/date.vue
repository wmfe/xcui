<template>
  <transition name="slide-up" @after-leave="$emit('dodestroy')">
    <div
      ...
      class="x-picker-panel x-date-picker"
      :class="[{
      ...
      <div class="x-picker-panel-body-wrapper">
        ...
        <div class="x-picker-panel-body">
          <div class="x-date-picker-time-header" v-if="showTime">
            <span class="x-date-picker-editor-wrap">
              <x-input
                :placeholder="t('x.datepicker.selectDate')"
                :value="visibleDate"
                size="small"
            ...
            <span class="x-date-picker-editor-wrap">
              <x-input
                ...
                :placeholder="t('x.datepicker.selectTime')"
                :value="visibleTime"
                size="small"
          ...
          <div class="x-date-picker-header" v-show="currentView !== 'time'">
            <button
              type="button"
              @click="prevYear"
            ...
            <span
              ...
              class="x-date-picker-header-label">{{ yearLabel }}</span>
            <span
              @click="showMonthPicker"
              v-show="currentView === 'date'"
              class="x-date-picker-header-label"
              :class="{ active: currentView === 'month' }">{{t(`x.datepicker.month${ month + 1 }`)}}</span>
            <button
              type="button"
              @click="nextYear"
          ...
          <div class="x-picker-panel-content">
            <date-table
              v-show="currentView === 'date'"
              @pick="handleDatePick"
              :year="year"
      ...
      <div
        class="x-picker-panel-footer"
        v-show="footerVisible && currentView === 'date'">
        <a
          href="JavaScript:"
          class="x-picker-panel-link-btn"
          @click="changeToNow">{{ t('x.datepicker.now') }}</a>
        <button
          type="button"
          class="x-picker-panel-btn"
          @click="confirm">{{ t('x.datepicker.confirm') }}</button>
      </div>
    </div>
  </transition>
</template>
...
<script>
import { formatDate, parseDate } from '../util/index';
...
export default {
    ...
    computed: {
        ...
        yearLabel() {
            ...
            const yearTranslation = this.t('x.datepicker.year');
            if (this.currentView === 'year') {
