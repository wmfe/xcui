<template>
  <table
    ...
    <tbody>
    <tr>
      <th v-if="showWeekNumber">{{ t('x.datepicker.week') }}</th>
      <th v-for="week in WEEKS">{{ t('x.datepicker.weeks.' + week) }}</th>
    </tr>
    <tr
      class="x-date-table-row"
      v-for="row in rows"
      :class="{ current: value && isWeekActive(row[1]) }">
      <td
        v-for="cell in row"
        :class="getCellClasses(cell)"
        v-text="cell.type === 'today' ? t('x.datepicker.today') : cell.text"></td>
    </tr>
    </tbody>
  </table>
</template>
...
export default {
    ...
    computed: {
        ...
        rows() {
            ...
            const dateCountOfLastMonth = getDayCountOfMonth(date.getFullYear(),
    ...
    methods: {
        getCellClasses(cell) {
            ...
            if (selectionMode === 'day' && (cell.type === 'normal' || cell.type === 'today')
                && Number(this.year) === this.date.getFullYear()
                && this.month === this.date.getMonth()
                && monthDate === Number(cell.text)) {
        ...
        getDateOfCell(row, column) {
            ...
            return new Date(startDate.getTime()
              + (row * 7 + (column - (this.showWeekNumber ? 1 : 0)) - this.offsetDay) * DAY_DURATION);
        },
